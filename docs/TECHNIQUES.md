...
| 29 | **Receipts: `savings_tracker` + `/savings`** | Turns auto_context's *invisible* savings into a visible, personal, accumulating number. A Stop hook credits context-os for every file Claude opened that it surfaced (a "hit"), estimates tokens saved (conservative 8k/hit, env-overridable), and maintains a ledger + cached total. The statusLine shows a live `💰 2.3M saved · 5d🔥` meter; `/savings` prints a dashboard — all-time, this week, hit-rate, day-streak, dollar value, rate-limit runway, and a copy-paste shareable card. **The feedback loop that makes the win felt.** | 17-check correctness gate in CI (`python3 python/evals/runners/savings_test.py`): hit-detection, no over-count, session-isolation, milestone, streak, fail-open. Tokens-saved under-claims vs the measured A/B by design. |
...
- **`savings_tracker.py` (Stop).** The visibility layer. auto_context already saves ~40% of first-turn tokens — but silently. This hook reads the transcript for files Claude opened, joins them against the suggestions auto_context logged that session, and credits each match as a "hit" (a Glob/Grep exploration the agent skipped). It maintains `.context-os/savings/{ledger.jsonl,total.json}` and prints a one-line receipt at session end (`receipt: 3 hits → ~24,000 tokens saved · 5-day streak`), plus a celebration when the running total crosses a milestone. The statusLine reads the cached total for a live meter; `/savings` (backed by `.context-os/scripts/savings_report.py`) renders the full dashboard + a shareable card. Estimate is deliberately conservative (8k/hit vs the ~21k measured in the live A/B). `CONTEXT_OS_SAVINGS=0` disables; `CONTEXT_OS_SAVINGS_PER_HIT` tunes the per-hit credit.
