...
if [ "${1:-}" = "--status" ]; then
  ...
  if [ -f .claude/hooks/dedup_guard.py ] && [ -f .claude/hooks/loop_guard.py ] && [ -f .claude/hooks/file_size_guard.py ] && [ -f .claude/hooks/session_profile.py ] && [ -f .claude/hooks/auto_context.py ] && [ -f .claude/hooks/prewarm.py ]; then
    check ok "python hooks" "9 installed (dedup, loop, size, smartread, smartbash, profiler, autocontext, prewarm, savings)"
  else
...
" 2>/dev/null && echo "  removed allowedTools from settings.json"
  ...
  for h in dedup_guard.py loop_guard.py file_size_guard.py smart_read.py smart_bash.py session_profile.py auto_context.py prewarm.py savings_tracker.py; do
...
# --- smart_bash.py (PreToolUse Bash: close the Bash blind spot) ---
cat > "$HOOK_DIR/smart_bash.py" <<'CONTEXT_OS_SMARTBASH_EOF'
#!/usr/bin/env python3
"""
...
if __name__ == "__main__":
    try:
        ...
        sys.exit(0)
CONTEXT_OS_SMARTBASH_EOF
chmod +x "$HOOK_DIR/smart_bash.py"
...
PYTHON_HOOKS=$(cat <<HOOKJSON
{
  "hooks": {
    "PreToolUse": [
      ...
      {"matcher": "Bash", "hooks": [{"type": "command", "command": "python3 '$PY_ABS/smart_bash.py'", "timeout": 3}]}
    ],
    "UserPromptSubmit": [
...
" 2>/dev/null && echo "  [$STEP/$TOTAL] installed 9 Python hooks (dedup, loop, size, smartread, smartbash, profiler, autocontext, prewarm, savings)" || echo "  [$STEP/$TOTAL] Python hooks install failed"
else
  printf '%s' "$PYTHON_HOOKS" | python3 -m json.tool > "$SETTINGS_LOCAL" 2>/dev/null || printf '%s' "$PYTHON_HOOKS" > "$SETTINGS_LOCAL"
  echo "  [$STEP/$TOTAL] installed 9 Python hooks (dedup, loop, size, smartread, smartbash, profiler, autocontext, prewarm, savings)"
fi
