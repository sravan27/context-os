...
| 25 | **`auto_context` (graph RAG)** | UserPromptSubmit hook — parses your prompt, looks up keywords/paths/symbols in `.context-os/repo-graph.json`, and prepends a compact `<context-os:autocontext>` block with `file:line · symbol (kind) · imports` candidates. **Claude's first turn starts with structure already in hand.** No embeddings, no server, ~50ms. Env-overridable. | **Measured on 32 hand-labeled prompts across Python + TS + Rust fixtures: aggregate P@3 = 0.583, R@3 = 0.578, MRR = 0.922, coverage = 1.00** — **+0.094 P@3 / +0.359 MRR lift over naive filename baseline**. Session-replay simulation on the same 32 prompts shows **80.2% fewer tokens to first relevant file (wins 32/32, mean block cost 56 tok)**. Reports: `python/evals/reports/autocontext-eval.md`, `session-replay.md`. CI-gated, artifacts uploaded every main run. |
...
Techniques 1–17, 20–28 install via `setup.sh` (shell + Python stdlib only). Techniques 18–19 require the optional Rust binary.
